...
- `--character <角色名>`：手动指定角色（默认按提示词精确文字匹配自动注入）
- `--character-image <图片路径>`：临时覆盖角色参考图（仅本次生成生效）
...
```text
python "<脚本路径>" generate "洛天依V4公式服演唱会全身" --composition full-body
```
...
config.json 的 `characters` 段保存角色名 → 设定文字（+ 可选参考图 `image`），默认已含洛天依 V4 公式服（别名 `洛天依-V4公式服` 指向同一条）。触发规则：

- 提示词中**精确文字匹配**到角色名（或别名）即自动注入设定（含禁忌原文），如"画一张洛天依 V4 公式服全身"；
- 角色名后**紧跟"风格/风"不算点名**、不注入，如"洛天依风格原创"；
- 画其他人物/物品/风景零注入；表内没有的角色不注入，脚本给一句中文提示但不中断出图；
- `--character <名>` 手动兜底；`--character-image <路径>` 临时覆盖参考图。

角色参考图（v1 新增）：角色配置了 `image` 时，自动把参考图与提示词一起发给模型，提示词注明"保持图中人物设定，生成新场景"；参考图缺失/读不了时降级纯文字继续出图；尺寸比例不一致时用 Pillow 等比放画布、不拉伸。参考图只发送给本地代理，不上传任何云端服务。
