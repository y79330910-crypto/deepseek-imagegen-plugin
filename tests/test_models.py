...
class TestGenerateRequest(unittest.TestCase):
    ...
    def test_from_dict_rejects_wrong_types(self):
        ...
        with self.assertRaises(ValidationError):
            GenerateRequest.from_dict({"prompt": "x", "library_enabled": "yes"})

    def test_from_dict_rejects_removed_seed(self):
        with self.assertRaises(ValidationError):
            GenerateRequest.from_dict({"prompt": "x", "seed": 1})
    ...
    def test_request_to_dict_is_json_safe(self):
        req = GenerateRequest(prompt="x", images=["a.png"])
...
class TestGenerateResult(unittest.TestCase):
    ...
    def test_generation_id_autogenerated_unique(self):
        a = GenerateResult(
            path="a.png", image_model_used="m",
            requested_size="1x1", actual_size="1x1", prompt_used="p",
        )
        b = GenerateResult(
            path="b.png", image_model_used="m",
            requested_size="1x1", actual_size="1x1", prompt_used="p",
        )
